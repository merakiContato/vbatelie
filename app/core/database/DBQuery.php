...
class DBQuery
{
    ...
    // ALTERAÇÃO ------------------------------------------
    /**
     * @var array $notNullFields Campos que não podem estar nulos 
     */
     ...
     */
    public function __construct($tableName, $fieldsNames, $primaryKeys, $notNullFields) // $notNullFields FOI ADICIONADO AQUI.
    {
        ...
        $this->notNullFields = is_array($notNullFields) ? $notNullFields : [$notNullFields]; //ADICIONADO
    }
    ...
    public function update($values)
    {
        ...
        // Construir as cláusulas SET
        foreach ($this->fieldsName as $fieldName) {
            if (isset($values[$fieldName])) {
                $setClauses[] = "{$fieldName} = '{$values[$fieldName]}'";
            } else {
                // Adicione um log ou mensagem de erro para diagnóstico
                // e continue para o próximo campo se apropriado.
            }
        }
        ...
        foreach ($this->primaryKeys as $primaryKey) {
            if (isset($values[$primaryKey])) {
                $where->addCondition("AND", $primaryKey, "=", $values[$primaryKey]);
            } else {
        ...
        error_log("Consulta SQL de Atualização: $sql");
        error_log("Valores das chaves primárias: " . print_r($values, true));

        try {
            echo "Consulta SQL de Atualização: $sql";
            return $this->conn->query($sql);
        } catch (PDOException $error) {
            error_log("Erro durante a execução da consulta SQL: " . $error->getMessage());
            if ($error->getCode() == 23000) {
                throw new Exception('Violação de chave única ou estrangeira.');
            }
            throw $error;
    ...
    public function delete($primaryKeyValue)
{
    ...
    // Construir a cláusula WHERE com base nas chaves primárias
    foreach ($this->primaryKeys as $primaryKey) {
        $where->addCondition("AND", $primaryKey, "=", $primaryKeyValue);
    }

    $sql .= " " . $where->build();

    error_log("Query SQL para exclusão: $sql");
