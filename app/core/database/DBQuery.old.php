...
class DBQuery
{
    ...
    public function update($values)
    {
        ...
        for ($i = 0; $i < count($values); $i++) {
            if (isset($this->fieldsName[$i]) && isset($values[$this->fieldsName[$i]])) {
                $setClauses[] = "{$this->fieldsName[$i]} = '{$values[$this->fieldsName[$i]]}'";
            } else {
                // Adicione um log ou mensagem de erro para diagnóstico
                // e continue para o próximo índice se apropriado.
            }
        }
...
/*
        ...
        $where->addCondition("AND", $this->primaryKeys[0], "=", $values[$this->fieldsName[0]]);
        //Aqui ele adiciona que o valor da linha sempre vai ser a primeira, ou seja, o 0
        //e tranforma a primary key e transforma em um array, para ela poder ter varios valores
        $sql .= " " . $where->build();
